        ...
        <div className='grid grid-row lg:grid-cols-2 gap-4'>
            <div className='grid justify-items-center'>
                <Image 
                    ...
                    width={500} height={750} alt={movie.alkuperainennimi} 
            ...
            <div className='grid grid-rows-5 gap-2'>
                <div className='row-span-2'>
                    ...
                    <h3 className='text-xl'>
                        {((movie.nimi != movie.alkuperainennimi) ? `(${movie.alkuperainennimi})` : null)}
                    </h3>
                    <div className='relative h-20 overflow-hidden'>
                        
                        {(tmdbData.translations.translations.filter(function(translation : any) { return translation.name === 'suomi'; }).length > 0)
                            ? <p className='kuvaus'>{tmdbData.translations.translations.find((translation: { name: string; }) => translation.name === "suomi").data.overview}</p>
                    ...
                    <div className='text-right'>
                        <label htmlFor="descriptionModal" className="cursor-pointer text-blue-600 underline">
                            Lue lisää...
                        </label>
                    </div>
                </div>
                <div className='grid grid-row row-span-2'>
                    ...
                    <p>{(movie.kestomin < 60) ? `${movie.kestomin}m` : toHoursAndMinutes(movie.kestomin)}</p>
                    <p>Näyttelijät: {tmdbData.credits.cast.slice(0, 5).map(function(actor: { name: string; }) {return actor.name;}).join(", ")}</p>
