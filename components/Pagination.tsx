  ...
  return (
    <div className="btn-group place-content-center mt-4">
      ...
      <div className="dropdown dropdown-top">
        <label tabIndex={0} className="btn w-52" style={{ borderRadius: 0 }}>
          Page {currentPage} of {pages.length}
        </label>
        <div
          tabIndex={0}
          className="dropdown-content text-white bg-gray-700 menu p-2 shadow w-52 overflow-y-scroll h-40"
        >
          <ul>
            {pages.map((page) => (
      ...
      <Link
        href={`${genre}/${currentPage + 1}/${sortOrder}`}
        className={currentPage === pages.length ? "btn btn-disabled" : "btn"}
      >
        »
      </Link>
    </div>
